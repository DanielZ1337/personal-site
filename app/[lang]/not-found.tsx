...
export default function NotFound() {
	...
	return (
		<div className='h-full w-full flex-1 flex flex-col justify-center items-center'>
			...
			<div className='bg-primary px-2 text-sm rounded rotate-12 absolute'>{dict?.errors.NotFound.page.title}</div>
			<button className='mt-5 group'>
				<div className='relative inline-block text-sm font-medium text-primary group active:text-primary-foreground focus:outline-none focus:ring'>
					...
					<span className='relative block px-8 py-3 bg-foreground dark:bg-black/90 border border-primary'>
						<Link
							href={`/${lang}`}
							className={'group-active:shadow-2xl group-active:shadow-primary'}
						>
							{dict?.errors.NotFound.page.message}
						</Link>
					</span>
				</div>
			</button>
