...
export default function LanguageSelector() {
	...
	const { data, isLoading } = useDictionary(langCode)

	useEffect(() => {
		if (!data || !langCode) return
		const name = data.languages[langCode as keyof typeof data.languages]
		setLangName(name)
	}, [data, langCode])
		...
		return (
			<div className='relative overflow-clip'>
				<div aria-label={langCode} className='absolute'>
					...
					{langCode && (
						<Image
							src={`/languages/${langCode}.svg`}
							alt=' Language'
							fill
							className='object-cover h-full absolute top-0 left-0 opacity-10'
						/>
					)}
				</div>
			</div>
